...
pub fn calculate_kinematics(
    ...
    if let Some(wp) = &waypoint {
        ...
        while new_heading > PI { new_heading -= 2.0 * PI; }
        while new_heading < -PI { new_heading += 2.0 * PI; }
    }
...
pub fn init(ctx: &ReducerContext) -> Result<(), String> {
    ctx.db.physics_timer().insert(PhysicsTimer {
        scheduled_id: 0,
        scheduled_at: ScheduleAt::Time(ctx.timestamp + std::time::Duration::from_millis(TICK_RATE_MS)),
    });
    Ok(())
}
...
#[reducer]
pub fn set_waypoint(ctx: &ReducerContext, ship_id: u64, target_x: f32, target_y: f32, target_speed: f32) -> Result<(), String> {
...
pub fn physics_tick(ctx: &ReducerContext, _timer: PhysicsTimer) -> Result<(), String> {
    ...
    for ship in ctx.db.ship().iter() {
        let (new_x, new_y, new_heading, new_speed, new_waypoint) = calculate_kinematics(
            ship.x, ship.y, ship.heading, ship.speed, ship.waypoint.clone(), dt
        );
...
mod tests {
    ...
    fn test_kinematics_turn_towards_waypoint() {
        let wp = Waypoint { x: 0.0, y: 10.0, target_speed: 10.0 };
        // Initial heading is 0 (facing right), waypoint is directly up (PI/2)
        // With MAX_TURN_RATE = 1.0, dt = 0.1, we can turn 0.1 radians
        let (_x, _y, heading, _speed, _next_wp) = calculate_kinematics(0.0, 0.0, 0.0, 10.0, Some(wp), 0.1);
    ...
    fn test_kinematics_arrive_at_waypoint() {
        let wp = Waypoint { x: 0.5, y: 0.0, target_speed: 0.0 };
        // We are within 1.0 of the waypoint
        let (x, y, _heading, _speed, next_wp) = calculate_kinematics(0.0, 0.0, 0.0, 10.0, Some(wp), 0.1);
    ...
    fn test_simulation_until_arrival() {
        let wp = Waypoint { x: 10.0, y: 10.0, target_speed: 10.0 };
        let mut x = 0.0;
        let mut y = 0.0;
        let mut heading = 0.0;
        let mut speed = 10.0;
        ...
        while current_wp.is_some() && ticks < 200 {
            let (new_x, new_y, new_heading, new_speed, new_wp) = calculate_kinematics(
                x, y, heading, speed, current_wp.clone(), dt
            );
            x = new_x;
            y = new_y;
            heading = new_heading;
